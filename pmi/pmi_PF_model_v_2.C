...
{ 
 double temp_dmudt = mulow * ((E0 / (k * pow(t, 2))) - ((sqrt(F) * beta) / pow(t, 2)));
 double exp_3 = (sqrt(F) * ((beta / t) - gamma)) - (E0 / (k * t));
...
{
 double exp_4 = (sqrt(F)*((beta / t) - gamma)) - (E0 / (k * t));
 dmudmulow = exp(exp_4);
}
...
{
 double temp_dmudF = (mulow * ((beta / t) - gamma)) / (2 * sqrt(F));
 double exp_5 = (sqrt(F) * ((beta / t) - gamma)) - (E0 / (k * t));
