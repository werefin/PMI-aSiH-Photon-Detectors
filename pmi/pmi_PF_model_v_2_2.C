...
protected:
  const double k; // Boltzmann constant [ev/K]
  const double F_min; /* minimum electric field to avoid divisions by zero and second exponential term "saturation" */
...
{
 double exp_1 = E0 / (k * t);
 double exp_2 = sqrt(abs(F) + F_min) * ((beta / t) - gamma);
...
{ 
 double temp_dmudt = mulow * ((E0 / (k * pow(t, 2))) - ((sqrt(abs(F) + F_min) * beta) / pow(t, 2)));
 double exp_3 = (sqrt(abs(F) + F_min) * ((beta / t) - gamma)) - (E0 / (k * t));
 dmudt = temp_dmudt * exp(exp_3);
}
...
{
 double exp_4 = (sqrt(abs(F) + F_min) * ((beta / t) - gamma)) - (E0 / (k * t));
 dmudmulow = exp(exp_4);
}
...
{
 double temp_dmudF = (mulow * ((beta / t) - gamma)) / (2 * sqrt(abs(F) + F_min));
 double exp_5 = (sqrt(abs(F) + F_min) * ((beta / t) - gamma)) - (E0 / (k * t));
 dmudF = temp_dmudF * exp(exp_5);
}
