...

{
  ...
  double temp_mu_e = (A_star * pot * pow(t, 2)) / (n * q * (abs(F) + F_min)); // no simplifications
...
{
 double temp_dmudpot = (A_star * pow(t, 2)) / (n * q * (abs(F) + F_min));
...
{
 double temp_dmudn = (A_star * pot * pow(t, 2)) / (pow(n, 2) * q * (abs(F) + F_min));
...
{
 double temp_dmudt_1 = (A_star * pot * pow(t, 2) * b) / (sqrt(abs(F) + F_min) * n * q);
 double temp_dmudt_2 = 2 * ((A_star * pot * t) / (n * q * (abs(F) + F_min)));
...
{
 double temp_dmudF_1 = (A_star * pot * pow(t, 3) * b) / (2 * n * q * ((abs(F) + F_min) * sqrt(abs(F) + F_min)));
 double temp_dmudF_2 = (A_star * pot * pow(t, 2)) / (n * q * pow((abs(F) + F_min), 2));
