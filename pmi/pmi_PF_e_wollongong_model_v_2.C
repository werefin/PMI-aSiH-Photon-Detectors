...

{
  ...
  double temp_mu_e = (A_star * pot * pow(t, 2)) / (n * q * F); // no simplifications
  // double temp_mu_e = (A_star * pow(t, 2)) / (n * q); -> considering simplifications
  mu = temp_mu_e * (exp(exponent) - 1);
}
...
{
 double temp_dmudpot = (A_star * pow(t, 2)) / (n * q * F);
 double exp_1 = (b * t) * (sqrt(F));
...
{
 double temp_dmudn = (A_star * pot * pow(t, 2)) / (pow(n, 2) * q * F);
 double exp_2 = (b * t) * (sqrt(F));
 dmudn = (-temp_dmudn) * (exp(exp_2) - 1);
}
...
{
 double temp_dmudt_1 = (A_star * pot * pow(t, 2) * b) / (sqrt(F) * n * q);
 double temp_dmudt_2 = 2 * ((A_star * pot * t) / (n * q * F));
 double exp_3 = (b * t) * (sqrt(F));
...
{
 double temp_dmudF_1 = (A_star * pot * pow(t, 3) * b) / (2 * n * q * (F * sqrt(F)));
 double temp_dmudF_2 = (A_star * pot * pow(t, 2)) / (n * q * pow(F, 2));
 double exp_4 = (b * t) * (sqrt(F));
