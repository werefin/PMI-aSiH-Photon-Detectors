...
{
  double temp_mu = A_star * pow(pot, 2) * t;
...
{
 double temp_dmudpot = 2 * A_star * t * pot;
...
{
 double temp_dmudt_1 = A_star * pow(pot, 2);
 double temp_dmudt_2 = (A_star * sqrt(abs(F) + F_min) * pow(pot, 2) * b)/t;
...
{
 double temp_dmudF = (A_star * b * pow(pot, 2) / (2 * sqrt(abs(F) + F_min)));
